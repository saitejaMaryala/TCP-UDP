...
int sending_data = 1; // 0 for receiving, 1 for sending
...
int main()
{
    ...
    char message[] = "The sender (client or server - both should be able to send as well as receive data) must divide the data (assume some text) into smaller chunks (using chunks of fixed size or using a fixed number of chunks).";
    ...
    // server_addr.sin_addr.s_addr = INADDR_ANY; // Assuming server is on the same machine, otherwise provide server IP
    ...
    int temp = 1;
    sendto(sockfd, &temp, sizeof(temp), 0, (struct sockaddr *)&server_addr, addr_len);
    ...
    while (1)
    {
        if (sending_data)
        {
            ...
            send_data(sockfd, &server_addr, msg);
            sending_data = 0;
        }
        else
        {
